...
def train(model, train_loader, optimizer,scheduler,loss_fn,augumentation,T_aug,epoch_number):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if config.mode == 'intersection_and_union':
            k = config.k
            l1 = loss_fn1(output1, ids1)  + k*loss_fn1(output1, un_diff_inter1)
            l2 = loss_fn2(output2, ids2) + k*loss_fn2(output2, un_diff_inter2)
            loss = l1 + l2
...
name = (f'Annotator_{config.annotator}_k={config.k}_Model_{config.model}_Augmentation_{config.augmentation}_Mode{config.mode}_Optimizer_{config.optimizer}_Scheduler_{config.scheduler}_Epochs_{config.epochs}_Batch_Size_{config.batch_size}_Start_lr_{config.lr}_Loss_{config.loss}_Timestamp_{timestamp}')
